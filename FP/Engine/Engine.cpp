    ...
    g(*glb)
{
    ...
    _dx(Lvl_Info, "Checking for ISO plugin");
    if (g.ISO.IsValid())
    {
	// Insert ISO track to the list
	_dx(Lvl_Info, "Creating ISO builder");
	iso = g.ISO->CreateISO(0);
	
	ASSERTS(iso != 0, "ISO module failed to create new ISO");
	_dx(Lvl_Info, "ISO Builder: %08lx", (iptr)iso);
	if (iso != 0)
	{
	    _dx(Lvl_Info, "Appending ISO to list of tracks");
	    VectorT< IFileReader* > &v = tracks().ObtainWrite();
	    v << iso;
	    tracks().Release();
	}
    }
...
Engine::~Engine()
{
    ...
    VectorT<IFileReader*> &vecTracks = tracks().ObtainWrite();
...
void Engine::readSettings()
{
    ...
    _dx(Lvl_Info, "Reading Export configuration");
...
{
#warning przeniesc to do globalsow.
    vecExpData.Empty();
    vecExpAudio.Empty();
    vecExpSession.Empty();

    for (unsigned int i=0; i<g.RWHooks.GetCount(); i++)
    {
	if (g.RWHooks[i]->isWriter())
	{
	    if (g.RWHooks[i]->isData())
	    {
		vecExpData << g.RWHooks[i]->getName();
	    }

	    if (g.RWHooks[i]->isAudio())
	    {
		vecExpAudio << g.RWHooks[i]->getName();
	    }

	    if (g.RWHooks[i]->isSession())
	    {
		vecExpSession << g.RWHooks[i]->getName();
	    }
	}
    }
}
...
IRegHook *Engine::findHook(const char *name, bool bWriter, HookType type)
{
    for (unsigned int i=0; i<g.RWHooks.GetCount(); i++)
    {
	if (bWriter == g.RWHooks[i]->isWriter())
	{
	    if (strcmp(name, g.RWHooks[i]->getName()) == 0)
	    {
		switch (type)
		{
		    case HType_Any:
...
{
    String res;
    VectorT< IFileReader* > &v = tracks().ObtainRead();
    iptr cnt = v.Count();
    tracks().Release();
...
EDtError Engine::addTrack(const char* name)
{
    IRegHook *regHook = 0;
    IFileReader *fileReader = 0;
    EDtError rc = DT_FileFormatNotSupported;

    _dx(Lvl_Info, "Checking new track (%s) against registered readers", (iptr)name);

    for (iptr i=0; i<g.RWHooks.GetCount(); i++)
    {
	_dx(Lvl_Info, "Checking if %s matches criteria (Reader && !Session)", (iptr)g.RWHooks[i]->getName());
	if ((g.RWHooks[i]->isReader()) && (!g.RWHooks[i]->isSession()))
	{
	    _dx(Lvl_Info, "-> Passed. Checking if can handle data");
	    if (g.RWHooks[i]->checkFile(name, rc))
	    {
		_dx(Lvl_Info, "-> All fine! selecting %s as reader.", (iptr)g.RWHooks[i]->getName());
		regHook = g.RWHooks[i];
		break;
	    }
	}
    ...
    if (0 != fileReader)
    {
	_dx(Lvl_Info, "-> Adding new file reader (%08lx) to queue", (iptr)fileReader);
	VectorT< IFileReader* > &v = tracks().ObtainRead();
	v << fileReader;
	tracks().Release();
	thrOperations->DoAsync(Cmd_Update, new Event(Cmd_Update));
    }

    return rc;
}

void Engine::addTrack(IFileReader *f)
{
    VectorT<IFileReader*> &vecTracks = tracks().ObtainWrite();
    vecTracks << f;
    tracks().Release();
...
void Engine::remTrack(IFileReader* track)
{
    VectorT<IFileReader*> &vecTracks = tracks().ObtainWrite();
    vecTracks >> track;
    tracks().Release();
...
RWSyncT< VectorT<IFileReader*> > &Engine::tracks()
{
    return hTracks;
}
...
void Engine::addISOItem(const char* path)
{
	...
	dl = new JobISOAddItem(g, drv, iso, path);
...
void Engine::remISOItem(ClElement* elem)
{
	...
	dl = new JobISORemItem(g, drv, iso, elem);
	thrOperations->DoAsync(Cmd_SendEvent, new Event(Eng_DisableISO));
	thrOperations->DoAsync(Cmd_ExecuteJob, new Event(dl));
	thrOperations->DoAsync(Cmd_SendEvent, new Event(Eng_EnableISO));
    }
...
void Engine::makeISODir(const char* path)
{
	...
	dl = new JobISOMakeDir(g, drv, iso, path);
	thrOperations->DoAsync(Cmd_SendEvent, new Event(Eng_DisableISO));
	thrOperations->DoAsync(Cmd_ExecuteJob, new Event(dl));
	thrOperations->DoAsync(Cmd_SendEvent, new Event(Eng_EnableISO));
    }
