...
JobUpload::JobUpload(Globals &glb, iptr drive, RWSyncT< VectorT<IFileReader*> > &trks, bool master, bool closedisc) :
    Job(glb, drive),
    tracks(trks),
...
void JobUpload::execute()
{
	...
	for (currTrack=0; currTrack<items.Count(); currTrack++)
	{
	    ...
	    IFileReader*trak    = items[currTrack].fr;
		...
		while (seccnt != 0)
		{
		    ...
		    res = trak->readData(item, memblk, xfer);
