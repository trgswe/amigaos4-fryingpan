...
class track
{
   ...
   VectorT<class index*>   indices;       // indices
...
public:
   ...
   void addIndex(class index *i)
   {
      indices << i;
   }

   class index *getIndex(int32 i)
   {
      return indices[i];
   }
...
#warning no postgap
   ...
   uint32 read(void* &mem, uint32 len)
   {
      ...
      while ((unsigned)curridx < indices.Count())
      {
...
bool                 rCUESession::readData(const IOptItem* item, void* mem, int len)
{
   if (item->getItemType() != Item_Track)
   {
      ...
      if ((unsigned)currtrk == tracks.Count())
         return false;
...
bool                 rCUESession::setUp(const IOptItem*)
{
...
iptr                 rCUESession::getBlockCount()
{
   return blockCount;
}
...
iptr* rCUESession::getSettingsPage()
{
    if (0 == muiPage)
    {
	muiPage = VGroup,
	    Child,	    muiLabel("Nothing to configure"),
	End;
    }
