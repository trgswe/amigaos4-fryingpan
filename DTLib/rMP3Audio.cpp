...
void                 rMP3Audio::readHeaders(EDtError &rc)
{
    ...
    */
   {
      ...
      {
         stream_size -= 128;
         _D(Lvl_Info, "%s: found ID3v1 tag. Structure size: 128 bytes", (iptr)__PRETTY_FUNCTION__);
      }
      else if ((x[118] == '3') && (x[119] == 'D') && (x[120] == 'I'))
      {
         stream_size -= 10;
         _D(Lvl_Info, "%s: found ID3v2.%ld.%ld footer. Structure size: 10 bytes", (iptr)__PRETTY_FUNCTION__, x[121], x[122]);
      }
      ...
      if ((x[0] == 'I') && (x[1] == 'D') && (x[2] == '3'))
      {
         ...
         _D(Lvl_Info, "%s: found ID3v2.%ld.%ld tag. Structure size: %ld bytes", (iptr)__PRETTY_FUNCTION__, x[3], x[4], sz);
...
iptr                  rMP3Audio::access(void* , MPEGA_ACCESS *acc)
{
   register uint32 d;
   ...
   switch (acc->func)
   {
      case MPEGA_BSFUNC_OPEN:
         ...
         current_location = 0;
         _D(Lvl_Info, "%s: MPEGA_OPEN - Opening virtual stream. Current location: %ld", (iptr)__PRETTY_FUNCTION__, current_location);
         return (iptr)this;
      ...
      case MPEGA_BSFUNC_CLOSE:
          ...
          */
         _D(Lvl_Info, "%s: MPEGA_CLOSE - Disposing virtual stream.", (iptr)__PRETTY_FUNCTION__);
         return 0;
      ...
      case MPEGA_BSFUNC_READ:
         ...
         {
            _D(Lvl_Info, "%s: MPEGA_READ - no more data", (iptr)__PRETTY_FUNCTION__);
            return 0;
         }

         d = DOS->Read(handle, acc->data.read.buffer, (unsigned)acc->data.read.num_bytes <? (stream_size - current_location));
         current_location += d;
         _D(Lvl_Info, "%s: MPEGA_READ - Read %ld bytes. Advancing position to %ld.", (iptr)__PRETTY_FUNCTION__, d, current_location);
         return d;

      case MPEGA_BSFUNC_SEEK:
         current_location = ((unsigned)acc->data.seek.abs_byte_seek_pos <? stream_size);
         _D(Lvl_Info, "%s: MPEGA_SEEK - Seeking to relative location %ld.", (iptr)__PRETTY_FUNCTION__, current_location);
         DOS->Seek(handle, current_location + stream_start, OFFSET_BEGINNING);
         return 0;
   }
   
   _D(Lvl_Warning, "%s: MPEGA_xxx - Your MPEGA library just asked me to do something I never heard of (%ld)", (iptr)__PRETTY_FUNCTION__, acc->func);
